...
class LegalMonetaryTotal(Xmleable):
    ...
    def fix_values(self):
        ...
        if type(self.tax_inclusive_amount) in [float, int]:
            self.tax_inclusive_amount = TaxInclusiveAmount(self.tax_inclusive_amount)
        ...
        if type(self.allowance_total_amount) in [float, int]:
            self.allowance_total_amount = AllowanceTotalAmount(self.allowance_total_amount)
        if type(self.charge_total_amount) in [float, int]:
            self.charge_total_amount = AllowanceTotalAmount(self.charge_total_amount)

    def validate(self, errs, obs):
        assert self.prepaid_amount is None or type(self.prepaid_amount) == PrepaidAmount
        assert self.payable_amount is None or type(self.payable_amount) == PayableAmount
        assert self.line_extension_amount is None or type(self.line_extension_amount) == LineExtensionAmount
        assert self.tax_inclusive_amount is None or type(self.tax_inclusive_amount) == TaxInclusiveAmount
        assert self.allowance_total_amount is None or type(self.allowance_total_amount) == AllowanceTotalAmount
        assert self.charge_total_amount is None or type(self.charge_total_amount) == ChargeTotalAmount
...
class RequestedMonetaryTotal(Xmleable):
    ...
    def fix_values(self):
        ...
        if type(self.payable_rounding_amount) in [float, int]:
            self.payable_rounding_amount = PayableRoundingAmount(self.payable_rounding_amount)
    ...
    def validate(self, errs, obs):
        ...
        assert self.allowance_total_amount is None or type(self.allowance_total_amount) == AllowanceTotalAmount
        assert self.charge_total_amount is None or type(self.charge_total_amount) == ChargeTotalAmount
        assert self.payable_rounding_amount is None or type(self.payable_rounding_amount) == PayableRoundingAmount
