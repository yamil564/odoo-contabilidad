...
class PartyLegalEntity(Xmleable):
    ...
    def validate(self, errs, obs):
        assert self.registration_name is None or type(self.registration_name) == RegistrationName
        assert self.registration_address is None or type(self.registration_address) == RegistrationAddress
...
class Party(Xmleable):
    ...
    def validate(self, errs, obs):
        assert self.party_identification is None or type(self.party_identification) == PartyIdentification
        assert self.party_name is None or type(self.party_name) == PartyName
        assert self.party_legal_entity is None or type(self.party_legal_entity) == PartyLegalEntity
        assert self.party_tax_schema is None or type(self.party_tax_schema) is PartyTaxScheme
