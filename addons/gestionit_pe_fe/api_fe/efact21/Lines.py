...
class SellersItemIdentification(Xmleable):
    ...
    def generate_doc(self):
        self.doc = default_document.createElement("cac:SellersItemIdentification")
...
class ItemClassificationCode(Xmleable):
    ...
    def generate_doc(self):
        self.doc = createElementContent("cbc:ItemClassificationCode", self.c_code)
        if self.add_attributes:
...
class CommodityClassification(Xmleable):
    ...
    def fix_values(self):
        if type(self.item_classification_code) == str:
            self.item_classification_code = ItemClassificationCode(self.item_classification_code)

    def generate_doc(self):
        self.doc = default_document.createElement("cac:CommodityClassification")
...
class Item(Xmleable):
    ...
    def fix_values(self):
        ...
        if type(self.seller_item_identification) == str:
            self.seller_item_identification = SellersItemIdentification(self.seller_item_identification)
        if type(self.commodity_classification) == str:
            self.commodity_classification = CommodityClassification(self.commodity_classification)

    def validate(self, errs, obs):
        assert self.description is None or type(self.description) == Description
        assert self.seller_item_identification is None or\
            type(self.seller_item_identification) == SellersItemIdentification
        assert self.commodity_classification is None or type(self.commodity_classification) == CommodityClassification
    ...
    def generate_doc(self):
        ...
        if self.seller_item_identification:
            self.doc.appendChild(self.seller_item_identification.get_document())
