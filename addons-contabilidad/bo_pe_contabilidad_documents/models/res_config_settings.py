...
class ResConfigSettings(models.TransientModel):
    ...
    property_account_receivable_fees_id=fields.Many2one('account.account',
        ...
        related='company_id.property_account_receivable_fees_id',readonly=False,
    ...
    group_account_fees_ME = fields.Boolean("Cuentas de Recibos por Honorario por Cobrar y por Pagar en Divisa",  implied_group='base.group_multi_currency', default=False)#,

    property_account_receivable_me_fees_id=fields.Many2one('account.account',
        string="Cuenta Recibo Honorarios a Cobrar ME",
        related='company_id.property_account_receivable_me_fees_id',
    ...
    def sync_account_fees_partners(self):
    	query_update="""
    		update res_partner set property_account_receivable_fees_id=%s,
    			property_account_payable_fees_id=%s 
    		where 
    			active=true and 
    			massive_update_account_fees=true """%(
    				self.property_account_receivable_fees_id.id,
    				self.property_account_payable_fees_id.id)
    ...
    def sync_account_me_fees_partners(self):
    	query_update="""
    		update res_partner set property_account_receivable_me_fees_id=%s,
    			property_account_payable_me_fees_id=%s 
    		where 
    			active=true and 
    			massive_update_account_fees_me=true """%(
    				self.property_account_receivable_me_fees_id.id,
    				self.property_account_payable_me_fees_id.id)
