...
class ChargeIndicator(Xmleable):
    ...
    def generate_doc(self):
        self.doc = createElementContent("cbc:ChargeIndicator",self.charge_indicator)
...
class AllowanceChargeReasonCode(Xmleable):
    ...
    def generate_doc(self):
        self.doc = createElementContent("cbc:AllowanceChargeReasonCode",self.allowance_charge_reason_code)
...
class MultiplierFactorNumeric(Xmleable):
    ...
    def fix_values(self):
        if type(self.multiplier_factor_numeric) == str:
            self.multiplier_factor_numeric = float(self.multiplier_factor_numeric)

    def generate_doc(self):
        self.doc = createElementContent("cbc:MultiplierFactorNumeric",self.multiplier_factor_numeric)
...
class AllowanceCharge(Xmleable):
    ...
    def fix_values(self):
        if type(self.charge_indicator) == bool:
            self.charge_indicator = ChargeIndicator("true" if self.charge_indicator else "false")
        if self.allowance_charge_reason_code in ["00","01","02"]:
            self.allowance_charge_reason_code = AllowanceChargeReasonCode(self.allowance_charge_reason_code)
        if type(self.multiplier_factor_numeric) == float:
            self.multiplier_factor_numeric = MultiplierFactorNumeric(self.multiplier_factor_numeric)
    ...
    def validate(self,errs,obs):
        ...
        assert type(self.allowance_charge_reason_code) == AllowanceChargeReasonCode
