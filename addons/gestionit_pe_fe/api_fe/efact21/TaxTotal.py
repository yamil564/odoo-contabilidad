...
class TaxAmount(Amount):
    def __init__(self, amount, currencyID="PEN"):
        super(TaxAmount, self).__init__(amount, currencyID, element_name="cbc:TaxAmount")


class TaxableAmount(Amount):
    def __init__(self, amount, currencyID="PEN"):
        super(TaxableAmount, self).__init__(amount, currencyID, element_name="cbc:TaxableAmount")


class PerUnitAmount(Amount):
    def __init__(self, amount, currencyID="PEN"):
        super(PerUnitAmount, self).__init__(amount, currencyID, element_name="cbc:PerUnitAmount")
...
class TaxExemptionReasonCode(Xmleable):
    ...
    def generate_doc(self):
        self.doc = createElementContent("cbc:TaxExemptionReasonCode", self.tax_exemption_reason_code)
...
class TaxCategory(Xmleable):
    ...
    def fix_values(self):
        ...
        if type(self.tax_exemption_reason_code) == str:
            self.tax_exemption_reason_code = TaxExemptionReasonCode(self.tax_exemption_reason_code)

    def validate(self, errs, obs):
        assert self.category_id is None or type(self.category_id) == CategoryID
        assert self.percent is None or type(self.percent) == Percent or self.per_unit_amount is None or type(self.per_unit_amount) == PerUnitAmount
        assert self.tax_scheme is None or type(self.tax_scheme) == TaxScheme
        assert self.tax_exemption_reason_code is None or type(self.tax_exemption_reason_code) == TaxExemptionReasonCode
...
class TaxSubtotal(Xmleable):
    ...
    def validate(self, errs, obs):
        assert self.taxable_amount is None or type(self.taxable_amount) == TaxableAmount or self.base_unit_measure is None or type(self.base_unit_measure) == BaseUnitMeasure
        assert self.tax_amount is None or type(self.tax_amount) == TaxAmount
        assert self.tax_category is None or type(self.tax_category) == TaxCategory 
